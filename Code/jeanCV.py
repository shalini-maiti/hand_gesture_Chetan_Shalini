...
#        points = cv2.findNonZero(self.binary_mask_image)
#        boundingRect = cv2.boundingRect(points)
#        bgdModel = np.zeros((1,65),np.float64)
#        fgdModel = np.zeros((1,65),np.float64)
#        mask = np.zeros(self.binary_mask_image.shape[:2],np.uint8)
#        cv2.grabCut(self.image, mask, boundingRect, bgdModel, fgdModel, 5, cv2.GC_INIT_WITH_RECT)
        
#        self.binary_mask_image = np.where((mask==2)|(mask==0),0,1).astype('uint8')
#        plt.imshow(self.binary_mask_image)
...
#        output = cv2.bitwise_and(self.image,self.image,mask = image_mask)
        ...
        self.binary_mask_image = image_mask
        points = cv2.findNonZero(self.binary_mask_image)
        boundingRect = cv2.boundingRect(points)
        bgdModel = np.zeros((1,65),np.float64)
        fgdModel = np.zeros((1,65),np.float64)
        ...
        self.binary_mask_image = np.where((mask==2)|(mask==0)|(mask==1),0,1).astype('uint8')
        ...
        #A binary mask is returned. White pixels (255) represent pixels that fall into the upper/lower.
        mask_YCbCr = cv2.inRange(self.YCbCr_image * self.binary_mask_image[:, :, np.newaxis], lower_YCbCr_values, upper_YCbCr_values)
        mask_HSV = cv2.inRange(self.HSV_image  * self.binary_mask_image[:, :, np.newaxis], lower_HSV_values, upper_HSV_values) 
